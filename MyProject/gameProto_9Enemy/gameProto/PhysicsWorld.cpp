...
void PhysicsWorld::AddBoxCollider(EnumPhysicsType type, BoxCollider *box)
{
	...
	//敵弾の攻撃判定処理
	if (type == EnumPhysicsType::EnumEnemyShot)
	{
		mEnemyShotBox.push_back(box);
	}
}
...
void PhysicsWorld::RemoveBoxCollider(BoxCollider* box)
{
	...
	//　敵弾の攻撃判定にいるか？
	auto iterBullet = std::find(mEnemyShotBox.begin(), mEnemyShotBox.end(), box);
	if (iterBullet != mEnemyShotBox.end())
	{
		mEnemyShotBox.erase(iterBullet);
		return;
	}
}
...
void PhysicsWorld::Collision()
{
	...
	//敵の弾とプレイヤーの攻撃ボックスのヒットを調べる
	PlayerABandEnemyBBTest();

	//敵の弾と壁とのヒットを調べる
	EnemyBBTestandBGTest();
}
...
void PhysicsWorld::DebugShowBox()
{
	...
	DrawBoxs(mPlayerAttackBox, Color::Red);		//自機攻撃用ボックスの描画
	DrawBoxs(mEnemyShotBox, Color::Red);		//自機攻撃用ボックスの描画
}
...
void PhysicsWorld::EnemyAndBGTest()
{
	...
	for (auto p : mPlayerBoxs)
	{
		for (auto e : mEnemies)
		{
			BoxCollider* player = p;
			BoxCollider* enemy = e;
			if (Intersect(player->GetWorldBox(), e->GetWorldBox()))
			{
				//プレーヤーの壁めり込み修正処理へ
				dynamic_cast<PlayerActor*>(player->GetOwner())->FixCollision(player, enemy);
			}
		}
	}
	...
	for (auto p : mPlayerBoxs)
	{
		for (auto e : mEnemies)
		{
			...
			{
				//プレーヤーの壁めり込み修正処理へ
				dynamic_cast<PlayerActor*>(player->GetOwner())->FixEnemyCollision(player, enemy);
			}
		}
	}
}
...
//敵の弾とプレイヤーの当たり判定
void PhysicsWorld::EnemyAttackAndPlayerTest()
{
	for (auto pa : mEnemyShotBox)
	{
		for (auto e : mPlayerBoxs)
		{
			if (Intersect(pa->GetWorldBox(), e->GetWorldBox()))
			{
				dynamic_cast<PlayerActor*>(e->GetOwner())->HitBullet(e, pa);
				dynamic_cast<EnemyShot*>(pa->GetOwner())->HitPlayer(pa, e);
			}
		}
	}
}

//敵の弾とプレイヤーの攻撃ボックスの当たり判定
void PhysicsWorld::PlayerABandEnemyBBTest()
{
	for (auto pb : mPlayerAttackBox)
	{
		for (auto eb : mEnemyShotBox)
		{
			if (Intersect(pb->GetWorldBox(), eb->GetWorldBox()))
			{
				dynamic_cast<EnemyShot*>(eb->GetOwner())->EnemyBBandPlayerAB(eb, pb);
				dynamic_cast<PlayerActor*> (pb->GetOwner())->PlayerABandEnemyBB(pb, eb);
			}
		}
	}
}
...
//敵弾と壁との当たり判定
void PhysicsWorld::EnemyBBTestandBGTest()
{
	for(auto eb : mEnemyShotBox)
	{
		for (auto bg : mBGBoxs)
		{
			if (Intersect(eb->GetWorldBox(), bg->GetWorldBox()))
			{
				dynamic_cast<EnemyShot*>(eb->GetOwner())->FixShotCollision(eb, bg);
			}
		}
