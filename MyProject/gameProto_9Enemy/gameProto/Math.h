...
namespace Math
{
	...
	inline float EaseIn(float percentage, float startVal, float endVal)
	{
		float c = endVal - startVal;
		return c * percentage * percentage + startVal;
	}

	inline float EaseOut(float percentage, float startVal, float endVal)
	{
		float c = endVal - startVal;
		return -c * percentage * (percentage - 2.0f) + startVal;
	}

	inline float EaseInOut(float percentage, float startVal, float endVal)
	{
		// t : 時間 b : 開始値  c:開始と終了の差分  d:合計時間
		//t /= d / 2.0
		//	if t < 1:
		//return c / 2.0 * t * t + b
		//	t = t - 1
		...
		float c = endVal - startVal;
		float t = percentage / (1.0f / 2.0f);
		if (t < 1.0f)
		{
			return c / 2.0f * t * t + startVal;
		}
		t = t - 1;
		return -c / 2.0f * (t * (t - 2.0f) - 1.0f) + startVal;
	}
}
