...
public:
	...
	void           PlayAnimation(EnemyStateEnum state, float rate = 1.0f);
	...
	void		   SetEnemyCount(int enemyCount) { mEnemyCount = enemyCount; }	//敵の残り数をセット
	...
	static int	   GetEnemyCount() { return mEnemyCount; }	//敵の残り数
...
protected:
	...
	int            mHitPoint    ; // 体力
	static int     mEnemyCount  ; //敵の残数
	static int     mNearEnemyCount; //近接用の敵の残数
